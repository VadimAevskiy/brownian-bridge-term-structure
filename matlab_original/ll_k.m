function [n] =ll_k(bigtheto) ;%ll_k(lamda, sigma, k);%  
% lamda=-0.061;%-0.1;%-0.4;%-0.01;%0.09879;
%  sigma=0.0037;%0.003;%0.02;%0.0001;
% k=1;%0.5;%0.4;
global q_mat
% 
lamda = bigtheto(1);
...
R008=emuweek(268:end,1);%27.12.1995:12.08.1998
...
for tau=2:T;% tau=T-t%  tau=T-t+1
       ...
       % A(n,tau)=lamda^2/2 + A(n-1,tau-1) - (lamda+B(n-1,tau-1)*sigma)^2/2;
           ...
           % B(n,tau) =( tau)/2;
                    ...
                    A(n,tau)=lamda.^2/2 + A(n-1,tau-1) - (lamda+B(n-1,tau-1).*exp(sigma)).^2/2;
         end
end
...
%     
        ...
        Y=Y(87:end,:); %92
        dthat1=dthat(87:end,:);% 20.08.1997:12.08.1998 (354)
        er= dthat1- Y;
%         figure;
%         plot(q_mat,er);
        ...
        %n2=(norm(er2)./exp(sigma))^2;
          S1=sum(sum(er.^2));%./(exp(sigma))^2;
          n=S1;
%        T1=length(er2);
%         S2=sum(er2.^2)/(exp(sigma))^2;
%         S3=+ 2*T1*sigma;
%                n = S1 + S2+S3;
%         n=n1+n2;
